...
Ibles.views.BaseAuth = Backbone.View.extend({ 
    ...
    resetAction: function($form, successCallback, errorCallback) {
        ...
        Ibles.session.resetPassword({
            ...
            success: function(resetData){
                self.loginActionWithParams(resetData["screenName"], password, buttonTrigger, successCallback, errorCallback);
            },
            error: function(data){
    ...
    loginActionWithParams: function(username, password, buttonTrigger, successCallback, errorCallback){
        ...
        Ibles.session.login({
            ...
            success: function(data){
                if (successCallback) successCallback(_.extend(data, {
                    screenName: username,
                    password: password
                }));
            },
    ...
    signupAction: function($form, successCallback, errorCallback, extraParams){
        ...
        Ibles.session.register(_.extend({
            ...
            success: function(signupData) {
                self.loginActionWithParams(screenName, password, buttonTrigger, successCallback, errorCallback);
            },
            error: function(data) {
                if (errorCallback) errorCallback(self.makeErrorMessages(data));
                buttonTrigger.button('reset');
    ...
    postAuthRedirect: function(authResponse) {
    ...
    registerVisibleHandler: function($form, callback) {
        $form.each(function(){
            var $form = $(this),
            ...
            if (!inModal) {
                ...
                var modalShownCallback = function() {
                ...
                $modalContainer.on('shown', modalShownCallback);                
            }
        });
    },
    ...
    setUTMZCookie: function(){
        ...
        try {
            ...
            try {
                //sometimes clearing local storage fixes this problem after the browser restarts
                localStorage.clear()
            } catch (clearError){
                //probably corrupted localstorage- do nothing
            }
        }
    }
});
...
Ibles.views.Signup = Ibles.views.BaseAuth.extend({
    ...
    submitSignup: function(form) {
        ...
        this.signupAction($form, function(data){
            self.postAuthRedirect(_.extend(data, self.options, {
                goingPro: $form.hasClass('going-pro'),
                sourcea: 'pdfSignupModal',
                sourceaUrl: window.location.pathname
            }));
        }, function(error){
    ...
    postAuthRedirect: function(authResponse) {
        var options = this.options,
            goingPro = authResponse.goingPro,
            proItem = Ibles.getQueryStringParam("proItem"),
            sourcea = Ibles.getQueryStringParam("sourcea"),
            sourceaUrl = Ibles.getQueryStringParam("sourceaUrl"),
            nextPageValue = Ibles.getQueryStringParam(options.nextPageParameter),
            customizeUrl="/feed/customize/get_started/?";

        if (goingPro) {
            window.location = "/payment/pay/?sourcea=" + authResponse.sourcea +
                "&sourceaUrl=" + authResponse.sourceaUrl +
                "&" + options.nextPageParameter + "=" + authResponse.sourceaUrl +
                "?download=pdf";
        } else if (proItem) {
            window.location = "/payment/pay/?proItem=" + proItem + "&sourcea=" + sourcea + "&sourceaUrl=" + sourceaUrl;
        } else if (sourceaUrl) {
            window.location = customizeUrl + options.nextPageParameter + "=" + sourceaUrl;
        } else if (nextPageValue){
            window.location = customizeUrl + options.nextPageParameter + "=" + nextPageValue;
        } else {
            window.location = customizeUrl.slice(0,-1);
        }
    }    
});
...
Ibles.views.Login = Ibles.views.BaseAuth.extend({
    ...
    submitLogin: function(form) {
        var $form = $(form),
            self = this;
        this.loginAction($form, function(authResponse) {
            self.postAuthRedirect(authResponse);
        }, function(error) {
...
Ibles.views.Reset = Ibles.views.BaseAuth.extend({
    ...
    submitResetSuccess: function(authResponse) {
        this.postAuthRedirect(authResponse);
    }
});
...
Ibles.AuthFlow = Ibles.views.BaseAuth.extend({
    events: {
        ...
        'click .verification-required': 'showVerifyModal'
    },
    ...
    initialize: function(options) {
        ...
        sessionReady(function(sessionModel){
            if (sessionModel.authenticated() && !sessionModel.get('emailVerified')){
                new Ibles.views.EmailVerificationModal();
                that.removeNewCollectionModal();
                if ($.cookie('verifyHeader') !== 'false' && !options.disableEmailVerifyWidget){
    ...
    showPro: function(e) {
        ...
        this.hideVisibleModals();
        if (!Ibles.session.authenticated()) {
            var modal = $('.auth-gopro-with-signup-modal:not(.inline)');
            modal.modal('show');
            ...
            setTimeout(function(){
                ...
                modal.removeClass("fade").removeClass("in");
                modal.css({
                    display: "block",
                    position: "absolute",
                    top: offset.top
                ...
                setTimeout(function(){
                    modal.addClass("fade").addClass("in");
                },500);
            },500);
        }
        else if (!Ibles.session.isPro() && !Ibles.session.isOld()) {
            $('.auth-gopro-modal:not(.inline)').modal('show');
        }
    },

    showVerifyModal: function(e){
        if (Ibles.session.authenticated() && Ibles.session.get('emailVerified') === false) {
            if (typeof e !== 'undefined')
                e.preventDefault();
            this.hideVisibleModals();
    ...
    removeNewCollectionModal: function(){
        $('#new-collection-modal').remove();
        //otherwise links to make a new collection modal show both the email verification modal AND the new collection modal
    }    
});
