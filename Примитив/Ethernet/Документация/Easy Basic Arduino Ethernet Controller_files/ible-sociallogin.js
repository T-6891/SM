    ...
    IbleSocialLoginProvider.prototype = {
        ...
        oauthPostLogin: function(callbackUrl) {
            var buttons = $(this.loginSelector),
                goingPro = $(this.loginSelector).hasClass('going-pro');
                self = this;
            ...
            $.ajax({
                ...
                statusCode:{
                    ...
                    201:function(){
                        self.postSignupRedirect(goingPro);
                    }
                },
                error: function(){
        ...
        postSignupRedirect:function(goingPro){
            var currentUrl = window.location.href,
                authPaths = this.options.authPaths,
                currentPath = window.location.pathname,
                nextPageValue = this.getUrlParam(currentUrl, this.options.nextPageParameter),
                onHomePage = false,
                onAuthPath = false,
            ...
            onHomePage = !!currentPath.match(/^\/$|^\/index/)

                if (goingPro) {
                    window.location = "/payment/pay/?sourceaUrl=" + currentPath +
                        "&" + this.options.nextPageParameter + "=" + currentPath +
                        "?download=pdf";
                } else if (nextPageValue) {
                    window.location = customizeUrl + this.options.nextPageParameter + "=" + nextPageValue;
                } else if (onHomePage || onAuthPath) {
                    window.location = customizeUrl.slice(0,-1);
                } else {
                    window.location = customizeUrl + this.options.nextPageParameter + "=" + currentPath;
                }
